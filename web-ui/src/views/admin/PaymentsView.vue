  ...
  <div class="admin-page">
    ...
    <el-card>
      <div class="search-bar">
        <el-input v-model="paymentNo" placeholder="搜索支付单号" style="width: 220px" clearable @clear="fetchPayments" @keyup.enter="fetchPayments" />
      ...
      <el-table :data="payments" v-loading="loading" stripe>
        ...
        <el-table-column label="支付时间" width="170">
          <template #default="{ row }">
            {{ row.payTime ? row.payTime.substring(0, 16) : '-' }}
          </template>
        </el-table-column>
...
const statusMap: Record<string, string> = {
  PENDING: '待支付', SUCCESS: '支付成功', FAILED: '支付失败', REFUNDED: '已退款'
}

function statusType(s: string) {
  return { PENDING: 'warning', SUCCESS: 'success', FAILED: 'danger', REFUNDED: 'info' }[s] || ''
}
